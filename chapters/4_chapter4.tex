...
\begin{equation}
	...
	\label{紧凑观测器形式}
\end{equation}
接下来给出一些不等式来支撑方法的推导。首先考虑杨不等式\cite{1934Inequalities}：
\begin{equation}
	x_1^\top x_2 \le \frac{x_1^\top x_1}{2\mu} + \frac{\mu x_2^\top x_2}{2}.
	\label{杨不等式}
\end{equation}
对于$x_1 \in \mathbb{R}^n$，$x_2 \in \mathbb{R}^n$，以及任意正常数$\mu$，上述不等式成立。另外，不等式\cite{wyman1994linear}：
\begin{equation}
...
接下来利用不变集的性质分析无人机编队在收敛过程中的避碰和通信保持，即满足不等式约束(\ref{距离约束条件})。首先，根据邻域条件(\ref{初值邻域条件})定义位置集合：
\begin{equation}
	\mathcal{S}_e = \{
		\mathbf{p} \in \mathcal{U}_\mathbf{p} :
		\forall(i,j) \in \varepsilon,
		\rho_{e_1} < \|\mathbf{p}_i-\mathbf{p}_j\|< \rho_{e_2}.
	\}
\end{equation}
由于李雅普诺夫函数(\ref{李雅普诺夫函数})包含势函数项$\sum_{(i,j) \in \varepsilon} \gamma_{ij}$。参考文献\parencite{4739215} 和$\dot{V}$的负定性，可说明$\mathcal{S}_e$为不变集，其在编队收敛过程中持续满足收敛条件(\ref{距离约束条件})。因此，当系统状态满足条件(\ref{初值邻域条件})时，邻居无人机间能够避免发生碰撞同时保持通信的约束。
